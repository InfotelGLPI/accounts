         ...
         foreach ($accounts as $data) {
            ...
            if (isset($hash_id) && $aeskey->getFromDBByHash($hash_id) && $aeskey->fields["name"]) {
                ...
                echo Html::hidden("encrypted_password$accountID", array('value'        => $data["encrypted_password"], 
                                                                        ...
                                                                        'autocomplete' => 'off'));
                echo "<input type='text' id='hidden_password$accountID' value='' size='30' onClick='javascript:if (!check_hash()) { $(\"#hidden_password$accountID\").after(\"Wrong encryption key\", \"accounts\").remove(); } else { decrypt_password(\"$accountID\");}'>";

            } else {

                $url = $CFG_GLPI["root_doc"]."/plugins/accounts/front/account.form.php";
                echo "&nbsp;<input type='button' id='decrypt_link$accountID' onClick='javascript:if (!check_hash()) { alert(\"Wrong encryption key\", \"accounts\"); } else { var decrypted_password = decrypt_password(\"$accountID\"); $(\"#decrypt_link$accountID\").after(decrypted_password).remove();}' name='decrypte' value='".__s('Uncrypt', 'accounts')."' class='submit'>";
                echo Html::hidden("encrypted_password$accountID", array('value'        => $data["encrypted_password"],
                                                                        'id'           => "encrypted_password$accountID",
                                                                        'autocomplete' => 'off'));
