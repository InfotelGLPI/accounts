...
class Hash extends CommonDBTM
{
    ...
    public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
    {
        ...
        if (!$withtemplate) {
            switch ($item->getType()) {
                case __CLASS__:
                    $ong    = [];
                    $ong[2] = self::createTabEntry(__('Linked accounts list', 'accounts'));
                    $ong[3] = self::createTabEntry(__('Modification of the encryption key for all password', 'accounts'));
                    return $ong;
            }
        }
        return '';
    ...
    public function showForm($ID, $options = [])
    {
        ...
        */
        ...
        if ($ID < 1) {
            ...
            echo __('Please do not use special characters like / \ \' " & in encryption keys, or you cannot change it after.', 'accounts');
            echo "</div>";
            echo "</td>";
            echo "</tr>";
        }
    ...
    public static function updateHash($oldaeskey, $newaeskey, $hash_id)
    {
        ...
        $query_ = "SELECT *
                ...
                WHERE 1 ";
        $query_ .= $dbu->getEntitiesRestrictRequest(
            "AND",
